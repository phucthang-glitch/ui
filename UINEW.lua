...
function Library:CreateWindow(Setting)
	...
	-- Màu sắc cho từng tab (xoay vòng)
	local TAB_COLORS = {
		Color3.fromRGB(0, 200, 255),   -- cyan
		Color3.fromRGB(180, 80, 255),  -- tím
		Color3.fromRGB(255, 80, 150),  -- hồng
		Color3.fromRGB(80, 255, 160),  -- xanh lá
		Color3.fromRGB(255, 160, 40),  -- cam
		Color3.fromRGB(255, 220, 40),  -- vàng
		Color3.fromRGB(40, 180, 255),  -- xanh dương
	...
	local TopBar         = Instance.new("Frame")   -- chứa title + tab bar
	local TitleRow       = Instance.new("Frame")   -- dòng title
	local TabBar         = Instance.new("Frame")   -- dòng tab ngang
	local TabBarList     = Instance.new("ScrollingFrame")
	local TabBarLayout   = Instance.new("UIListLayout")
	local ControlList    = TabBarList              -- alias để search logic dùng được
	local UIListLayout   = TabBarLayout
	local ContentArea    = Instance.new("Frame")   -- vùng content bên dưới
	...
	Shadow.ImageColor3 = Color3.fromRGB(100, 30, 200)
	...
	-- Gradient stroke (dùng gradient trên UIStroke)
	...
	-- ── TOP BAR (title + tab bar) ────────────────────────────────
	TopBar.Name = "TopBar"
	TopBar.Parent = MainContainer
	TopBar.BackgroundColor3 = Color3.fromRGB(8, 8, 14)
	TopBar.BackgroundTransparency = 0
	TopBar.Size = UDim2.new(1, 0, 0, 70)   -- 28 title + 34 tabs + 8 padding
	...
	TitleLabel.Text = "<font color=\"rgb(0,200,255)\">" .. tostring(TitleNameMain) .. "</font>"
	...
	-- Search icon nhỏ bên phải tab bar
	local SearchFrame = Instance.new("Frame", TabBar)
	SearchFrame.AnchorPoint = Vector2.new(1, 0.5)
	SearchFrame.Position = UDim2.new(1, -6, 0.5, 0)
	SearchFrame.Size = UDim2.new(0, 110, 0, 26)
	SearchFrame.BackgroundColor3 = Color3.fromRGB(20, 15, 35)
	...
	sfStroke.Color = Color3.fromRGB(100, 60, 200)
	...
	SearchIcon.ImageColor3 = Color3.fromRGB(140, 100, 220)
	...
	SearchBox.Text = ""
	SearchBox.TextColor3 = Color3.fromRGB(200, 170, 255)
	SearchBox.TextSize = 11
	SearchBox.TextXAlignment = Enum.TextXAlignment.Left
	SearchBox.CursorPosition = -1

	-- ScrollingFrame chứa tabs ngang
	TabBarList.Name = "ControlList"
	TabBarList.Parent = TabBar
	TabBarList.BackgroundTransparency = 1
	TabBarList.Position = UDim2.new(0, 4, 0, 3)
	TabBarList.Size = UDim2.new(1, -125, 0, 34)   -- chừa chỗ search
	TabBarList.CanvasSize = UDim2.new(0, 0, 0, 0)
	TabBarList.ScrollBarThickness = 0
	TabBarList.ScrollingDirection = Enum.ScrollingDirection.X
	TabBarList.BottomImage = ""
	TabBarList.TopImage = ""
	TabBarList.BorderSizePixel = 0

	TabBarLayout.Parent = TabBarList
	TabBarLayout.FillDirection = Enum.FillDirection.Horizontal  -- TAB NẰM NGANG
	...
	Concacontainer.Position = UDim2.new(0, 0, 0, 70)
	Concacontainer.Size = UDim2.new(1, 0, 1, -70)
	...
	-- Đường kẻ phân cách topbar / content
	local contentLine = Instance.new("Frame", Concacmain)
	contentLine.Size = UDim2.new(1, 0, 0, 1)
	contentLine.BackgroundColor3 = Color3.fromRGB(60, 40, 100)
	contentLine.BackgroundTransparency = 0
	contentLine.BorderSizePixel = 0
	local clGrad = Instance.new("UIGradient", contentLine)
	...
	-- PageControl alias (cần cho compatibility – không dùng trực tiếp trong layout mới)
	local PageControl = TabBar  -- alias
	...
	UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		TabBarList.CanvasSize = UDim2.new(0, UIListLayout.AbsoluteContentSize.X + 8, 0, 0)
	end)
	...
	if not GlobalSearch then
		GlobalSearch = function(searchText)
			...
			for section, elements in pairs(elementsInSection) do
				...
				for _, ei in ipairs(elements) do if ei.elementFound then hasElementMatch = true end end
				for _, ei in ipairs(elements) do
					local c = ei.control
					if ei.elementFound then
						c.Element.Visible = true
						if ei.sectionFound or hasElementMatch then c.Section.Visible = true end
						foundTabs[c.TabName] = true
						c.TabButton.Visible = true
					elseif ei.sectionFound and not hasElementMatch then
	...
	-- ============================================================
	-- ADD TAB  –  HORIZONTAL TAB BAR REDESIGN
	-- ============================================================
	function Main_Function:AddTab(PageName, IconId)
		...
		-- Wrapper trong ControlList (TabBarList)
		local PageNameControl = Instance.new("Frame")
		local Frame           = Instance.new("Frame")
		local TabNameCorner   = Instance.new("UICorner")
		local Line            = Instance.new("Frame")  -- giữ lại tên cũ để click handler dùng
		...
		PageNameControl.Size = UDim2.new(0, 0, 1, 0)   -- width auto từ content
		PageNameControl.AutomaticSize = Enum.AutomaticSize.X
		...
		TabNameCorner.CornerRadius = UDim.new(0, 7)
		TabNameCorner.Parent = Frame

		local frameStroke = Instance.new("UIStroke", Frame)
		frameStroke.Color = tabColor
		frameStroke.Thickness = 1
		frameStroke.Transparency = 0.65

		-- Bottom indicator bar (active)
		Line.Name = "Line"
		Line.Parent = Frame
		...
		-- UIPadding cho Frame để tạo khoảng trống ngang
		local framePad = Instance.new("UIPadding", Frame)
		framePad.PaddingLeft = UDim.new(0, (IconId and IconId ~= "") and 26 or 10)
		framePad.PaddingRight = UDim.new(0, 10)
		framePad.PaddingTop = UDim.new(0, 2)
		framePad.PaddingBottom = UDim.new(0, 2)

		TabTitleContainer.Name = "TabTitleContainer"
		TabTitleContainer.Parent = Frame
		TabTitleContainer.BackgroundTransparency = 1
		TabTitleContainer.Size = UDim2.new(0, 0, 1, 0)
		TabTitleContainer.AutomaticSize = Enum.AutomaticSize.X
		...
		TabTitle.TextColor3 = Color3.fromRGB(110, 90, 150)  -- mặc định mờ
		...
		PageButton.MouseButton1Click:Connect(function()
			...
			for _, v in next, ControlList:GetChildren() do
				if not v:IsA('UIListLayout') then
					...
					local tc = TAB_COLORS[1]  -- fallback
					-- Tìm màu của tab này qua stroke
					...
					TweenService:Create(v.Frame, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), {
						BackgroundColor3 = isActive and Color3.fromRGB(20, 14, 38) or Color3.fromRGB(18, 12, 30),
						BackgroundTransparency = isActive and 0 or 0.1
					}):Play()
					if fs then
						TweenService:Create(fs, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), {
							Transparency = isActive and 0.2 or 0.65
						}):Play()
					end
					TweenService:Create(v.Frame.TabTitleContainer.TabTitle, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), {
						TextColor3 = isActive and tc or Color3.fromRGB(110, 90, 150)
					}):Play()
				end
			end
		end)
		...
		function pageFunction:AddSection(Section_Name, Toggleable, SectionGap, SectionColor)
			...
			Sectiontitle.TextColor3 = Color3.fromRGB(255, 210, 70)
			...
			local sectionFunction = {}
				...
				ToggleTitle.TextColor3 = Color3.fromRGB(130, 255, 185)
    ...
    ButtonClick()
            ...
            function sectionFunction:AddDropdownSection(Setting)
                ...
                Dropdowntitle.TextColor3 = Color3.fromRGB(255, 120, 180)
               ...
               local dropdownSectionFunction = {}
                ...
                function dropdownSectionFunction:AddSlider(Setting)
                    ...
                    SliderTitle.TextColor3 = Color3.fromRGB(255, 220, 80)
				...
				local function refreshlist(SortPairs)
					...
					if Selected then
                        for _, i in ipairs(OrderedList) do
							...
							SampleItemButton.TextColor3 = Color3.fromRGB(220, 200, 255)
...
function sectionFunction:AddKeyBind(Setting, Callback)
    ...
    BindButtonTitle.TextColor3 = Color3.fromRGB(100, 180, 255)
    ...
    Bindkey.TextColor3 = Color3.fromRGB(80, 220, 200)
    ...
    return keybindFunction
			...
			function sectionFunction:AddInput(idk, Setting)
				...
				Boxtitle.TextColor3 = Color3.fromRGB(80, 230, 200)
