...
function Library:CreateWindow(Setting)
	...
	if not GlobalSearch then
		GlobalSearch = function(searchText)
			...
			for _, control in pairs(getgenv().AllControls) do
				local en = string.lower(control.Name or "")
				local sn = string.lower(control.SectionName or "")
				local ef = string.find(en, searchText, 1, true) ~= nil
				local sf = string.find(sn, searchText, 1, true) ~= nil
				...
				table.insert(elementsInSection[control.Section], {
					control = control, elementFound = ef, sectionFound = sf
				})
			end
			local foundTabs = {}
			for section, elems in pairs(elementsInSection) do
				...
				for _, ei in ipairs(elems) do
					if ei.elementFound then hasMatch = true end
				end
				for _, ei in ipairs(elems) do
					local c = ei.control
					if ei.elementFound then
						c.Element.Visible = true
						if ei.sectionFound or hasMatch then c.Section.Visible = true end
						foundTabs[c.TabName] = true
						c.TabButton.Visible = true
					elseif ei.sectionFound and not hasMatch then
			...
			for tabName in pairs(foundTabs) do
				for _, tab in pairs(ControlList:GetChildren()) do
					if not tab:IsA("UIListLayout") and
						string.find(tab.Name, tabName, 1, true) then
						tab.Visible = true
					end
				end
			end
